...
MODULE_DESCRIPTION("Heartbeat mechanism that uses RDMA multicast");
MODULE_VERSION("0.1");
...
static void callback(pid_t pid, int imm_data) {
  heartbeat_send_multicast(imm_data);
  printk(KERN_INFO PR_PREFIX
         "Process %u invoked the callback by passing pointer %d as data\n",
         pid, imm_data);
  printk(KERN_INFO PR_PREFIX "Sending a notification over RDMA\n");
}
